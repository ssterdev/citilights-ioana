					...
					<li>
						...
						<span class="featured-status">
							...
							if($terms && !is_wp_error($terms)){
								$seperator = count($terms) > 1 ? ', ' : '';
								foreach ( $terms as $term ) {
									$link = get_term_link( $term, 'property_status' );
									if ( is_wp_error( $link ) )
										continue;
									echo '<a href="' . esc_url( $link ) . '">' .sprintf(__('For %s','noo'), $term->name ). '</a>' . $seperator;
								}
							}
							?>
						</span>						
...
class NooPropertyAdvancedSearchWidget extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		// re_property_advanced_map(false,__('Search', 'noo'),false,'search-vertical');
		$args = array(
			'gmap' => false,

...
if ( $bg_video && $bg_video_url !='') {
	...
	$bg_color_overlay = ( $bg_color_overlay != '' ) ? 'background-color: '.$bg_color_overlay.';' : '';
	
	$row_bg_no_parallax = $parallax ? "" : "background-image: url({$bg_image[0]});";
	$html[] = "<div  {$id} class=\"{$class}{$bg_image_class}{$border}\" style=\"{$row_bg_no_parallax}{$bg_color_overlay}{$padding_top}{$padding_bottom}{$custom_style}\">";
	$html[] = do_shortcode( $content );
	if($parallax)
		$html[] = "<div class=\"".$parallax_class."\" data-parallax=\"1\" data-parallax_no_mobile=\"".((string)$parallax_no_mobile === 'true' ? "0" : "1" ) ."\" data-velocity=\"".((float)$parallax_velocity)."\" style=\"background-image: url({$bg_image[0]});\" ></div>";
	$html[] = '</div>';

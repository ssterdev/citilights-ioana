...
<div class="user-sidebar-menu dashboard-sidebar">
	...
	<div class="user-menu-links">
		...
		<?php endif; ?>
		<?php if( !empty( $property_favorites ) ) : ?>
			<a href="<?php echo $property_favorites;?>" class="user-link <?php echo ( $page_template == 'property_favorites.php' ? 'active' : '' ); ?>">
				<i class="fa fa-heart"></i><?php _e('Favorites', 'noo');?>
			</a>
		<?php endif; ?>
	</div>
		...
		<form class="subscription_post" >
				...
				<div class="dropdown">
					...
					<ul class="dropdown-menu">
						...
						if(!empty($packages)) :
							...
							$plan_price = get_post_meta( $package->ID, '_noo_membership_price', true );
						?>
							<li>
			...
			<div class="form-group">
				...
				<label for="recurring_payment" class="checkbox-label">
					<input type="checkbox" id="recurring_payment" name="recurring_payment" value="1" /><?php _e('make payment recurring', 'noo'); ?>
				</label>
			</div>
			...
			<?php
			$noo_membership_page = re_get_agent_setting('noo_membership_page', '');

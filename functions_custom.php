...
if(!function_exists('noo_price_range_dropdown')):
	function noo_price_range_dropdown($min = 0, $max = 100000){

		$range_size = re_get_property_search_setting('price_range_size',10000);

		$price_range = isset( $_GET['p_price_range'] ) ? sanitize_text_field($_GET['p_price_range'] ) : '';

		$output[] = '<select class="form-control form-control-chosen" name="p_price_range" data-placeholder="'. esc_html__('All Price' ,'noo').'">';
		$output[]   = '<option value="">' .esc_html__('All Price','noo').'</option>';
		for ( $range_min = 0; $range_min < ( $max + $range_size ); $range_min += $range_size ) {
			...
			$min_price_output = $range_min;
			$price_output = $min_price_output . '-' . ( $range_min + $range_size );
			$selected = ($price_output == $price_range) ? 'selected' : '';
			$output[] = '<option value="' . $price_output . '" '. $selected .'>' . $price_output . '</li>';
...
if(!function_exists('noo_area_range_dropdown')):
	function noo_area_range_dropdown($min = 0, $max = 1000){

		$range_size = re_get_property_search_setting('area_range_size',50);

		$area_range = isset( $_GET['p_area_range'] ) ? sanitize_text_field($_GET['p_area_range'] ) : '';

		$output[] = '<select class="form-control form-control-chosen" name="p_area_range" data-placeholder="'. esc_html__('All Area' ,'noo').'">';
		$output[]   = '<option value="">' .esc_html__('All Area','noo').'</option>';
		for ( $range_min = 0; $range_min < ( $max + $range_size ); $range_min += $range_size ) {
			...
			$min_area_output = $range_min;
			$area_output = $min_area_output . '-' . ( $range_min + $range_size );
			$selected = ($area_output == $area_range) ? 'selected' : '';
			$output[] = '<option value="' . $area_output . '" '. $selected .'>' . $area_output . '</li>';

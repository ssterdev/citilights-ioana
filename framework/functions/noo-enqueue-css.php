...
if ( ! function_exists( 'noo_enqueue_site_style' ) ) :
	function noo_enqueue_site_style() {
		...
		if ( ! is_admin() ) {
			...
			do_action( 'noo_citilights_before_site_style' );
			...
			wp_register_style( 'noo-main-style', NOO_ASSETS_URI . "/css/{$main_css}.css", array(), NULL, 'all' );
			...
			do_action( 'noo_citilights_after_site_style' );
		}

		// ===== Support RTL
			// if ( is_rtl() ) {
			//   	wp_enqueue_style( 'noo-rtl', NOO_ASSETS_URI . '/css/rtl.css', array(), NULL, 'all' );
			// }

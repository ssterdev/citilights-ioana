...
if( !class_exists('nooPayPalFramework')) :
	class nooPayPalFramework
	{
		...
		private function __construct() {
			...
			$this->_currency = re_get_property_setting('currency');
		...
		public function getPaymentURL($order) {
			...
			if( isset($order['is_recurring']) && $order['is_recurring'] ) {
				...
				$this->_addField('cmd',			'_cart');
				$this->_addField('upload',			1);
				$this->_addField('paymentaction',	'sale');
				// $this->_addField('cmd',			'_xclick');

				// $this->_addField('cmd',			'_ext-enter');
				// $this->_addField('redirect_cmd',	'_xclick');
				$this->_addField('amount_1',		round($order['amount'], 2));
				$this->_addField('item_name_1',		$order['item_name']);
				$this->_addField('item_quantity_1',		1);
				$this->_addField('item_number_1',		$order['item_number']);
			}

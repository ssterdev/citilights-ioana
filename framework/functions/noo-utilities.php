...
if (!function_exists('get_sidebar_id')):
	function get_sidebar_id() {
		...
		if( is_tax('property_category')
			...
			$sidebar		= ( $default_layout != 'fullwidth' ) ? noo_get_option('noo_property_sidebar', '') : '';
			$queried_object = get_queried_object();
			$term_meta      = get_option( 'taxonomy_' . $queried_object->term_id );

			if( isset($term_meta['override_default_layout']) && (bool) $term_meta['override_default_layout'] ) {
				$layout		= isset( $term_meta['tax_layout'] ) ? $term_meta['tax_layout'] : $default_layout;
				$sidebar	= ( $layout != 'fullwidth' && isset( $term_meta['tax_sidebar'] ) ) ? $term_meta['tax_sidebar'] : $sidebar;
			}
...
if (!function_exists('get_page_layout')):
	function get_page_layout() {
		...
		if (is_page() || (is_front_page() && get_option('show_on_front') == 'page')) {
			...
			if(strpos($page_template, 'favorites') !== false) {
				return 'left_sidebar';
			}
		...
		if( is_tax('property_category')
			...
			$layout = noo_get_option('noo_property_layout','fullwidth');

			$queried_object = get_queried_object();
			$term_meta      = get_option( 'taxonomy_' . $queried_object->term_id );
			if( isset($term_meta['override_default_layout']) && isset( $term_meta['tax_layout'] ) ) {
				$layout = $term_meta['tax_layout'];
			}
...
if ( ! function_exists( 'noo_citilights_allowed_html' ) ) :

	function noo_citilights_allowed_html() {

		return apply_filters( 'noo_citilights_allowed_html', array(
            'a' => array(
                'href' => array(),
                'target' => array(),
                'title' => array(),
...
if ( ! function_exists( 'noo_citilights_html_content' ) ) :

	function noo_citilights_html_content( $content = '' ) {

		return wp_kses( $content, noo_citilights_allowed_html() );
...
if ( ! function_exists( 'noo_citilights_get_current_url' ) ) :

	function noo_citilights_get_current_url() {

		if ( !empty( $_SERVER['QUERY_STRING'] ) ) {
			$string = $_SERVER['QUERY_STRING'];
			if ( preg_match('/redirect_to/is', $string) ) {
				$url = explode( 'redirect_to=', $string );

				if ( !empty( $url[1] ) ) {
					return urldecode( $url[1] );
				}
		...
		return noo_current_url();

	}

endif;

if ( ! function_exists( 'noo_get_list_location_property' ) ) :

    function noo_get_list_location_property() {

        $locations = get_categories( array('orderby' => 'NAME', 'order' => 'ASC', 'taxonomy' => 'property_location', 'hide_empty' => false ) );
        $list = array();
        foreach ( (array)$locations as $location ) {
            $list[ $location->name ] = $location->term_id;
        }
        return $list;
    }

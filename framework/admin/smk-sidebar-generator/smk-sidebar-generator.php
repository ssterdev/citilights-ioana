...
class SMK_Sidebar_Generator {
	...
	*/
	public function __construct(){
		...
		$this->sbg_name = __('Sidebar Generator', 'noo');
		...
		if( 'themes.php' == $pagenow && isset( $_GET['page'] ) && $_GET['page'] == strtolower( __CLASS__ ) ){
			...
			wp_enqueue_script('jquery-ui-slider');
	 		...
	 		wp_localize_script('smk_sbg_scripts', 'smk_sbg_lang', array(
					'remove'        => __('Remove', 'noo'),
					'not_saved_msg' => __("You've made changes, don't forget to save.", 'noo'),
					'ok'            => __("Changes were saved successfully.", 'noo'),
					'fail'          => __("An unexpected error ocurred.", 'noo'),
					'created'       => __("The sidebar was successfully created.", 'noo'),
					's_exists'      => __("The sidebar already exists. Please change the name.", 'noo'),
					'empty'         => __("Please enter a name for this sidebar.", 'noo'),
					's_remove'      => __("Are you sure? If you remove this sidebar it can't be restored.", 'noo'),
					's_removed'     => __("Sidebar Removed", 'noo'),
					'data_imported' => __("Data imported successfully.", 'noo'),
	...
	*/
	...
	$text = array(
			__('Add new', 'noo'),//0
			__('Save Changes', 'noo'),//1
			__('Sidebar Name:', 'noo'),//2
			__('ID:', 'noo'),//3
			__('Shortcode:', 'noo'),//4
			__('Remove', 'noo'),//5
			__('Import', 'noo'),//6
	...
	echo '<div class="wrap smk_sbg_main_block">';
		...
		echo '<div class="sbg_clearfix">';
			...
			echo '<div class="sbg_grid_50">';
				...
				echo '<div class="smk_sbg_main_menu">
						<span data-id="tab_main_form" class="active">'. __('Sidebars', 'noo') .'</span>
						<span data-id="tab_export">'. __('Export', 'noo') .'</span>
						<span data-id="tab_import">'. __('Import', 'noo') .'</span>
						<span data-id="tab_how_to">'. __('How to use?', 'noo') .'</span>
				</div>';
			echo '</div>';
		echo '</div>';
		...
		echo '<div id="tab_export" class="smk_sbg_tab additional">';
			//Export form
			echo '<div class="smk_sbg_label">' . __('Copy text from textarea:','noo') .'</div>';
		...
		echo '<div id="tab_import" class="smk_sbg_tab additional">';
			//Import form
			echo '<div class="smk_sbg_label">' . __('Paste exported data in textarea:','noo') .'</div>';
		...
		echo '<div id="tab_how_to" class="smk_sbg_tab additional">';
			//Import form
			echo '<h2>' . __('How to use?','noo') .'</h2>';
			echo '<h3>' . __('Shortcode:','noo') .'</h3>';
			echo '<p>' . __('Paste the shortcode anywhere you want, in posts, pages etc. If the input accept shortcodes, then the sidebar will be displayed.','noo') .'</p>';
			echo '<pre>[smk_sidebar id="SIDEBAR_ID"]</pre>';
			echo '<h3>' . __('Function:','noo') .'</h3>';
			echo '<p>' . __('You can use the built-in function, but for this you should modify theme files and make sure to check if the function exists before use it.','noo') .'</p>';
			echo '<pre>
if(function_exists("smk_sidebar"){
 	smk_sidebar("SIDEBAR_ID");
}
			</pre>';
			echo '<h3>' . __('WP Native function:','noo') .'</h3>';
			echo '<p>' . __('You can use the built-in function <em>smk_sidebar</em>, but anyways I recommend using WP native function to avoid conflicts.','noo') .'</p>';
			echo "<pre>
if(function_exists('dynamic_sidebar') && dynamic_sidebar('SIDEBAR_ID')) : 
endif;
			</pre>";

			echo '<h3>' . __('For more info visit the following links:','noo') .'</h3>';

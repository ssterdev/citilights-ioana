...
function noo_render_metabox_fields ( $post, $id, $type, $meta, $std, $field = null ) {
	switch( $type ) {
		...
		case 'gallery':
			...
			if ( $meta != '' ) {
				...
				foreach ( $image_ids as $image_id ) {
					$output .= wp_get_attachment_image( $image_id, 'thumbnail', false, array( 'data-imageid' => $image_id ) );
				}
			}
			...
			<script>
				jQuery(document).ready(function($) {
					function updateOrder_<?php echo $id; ?>( e, u ) {
						var imageList = [];
			            $('#<?php echo $id; ?>').siblings('.noo-thumb-wrapper').find('img').each(function(){
			                if( this !== undefined && $(this).data('imageid') !== undefined ) {
			                    imageList.push($(this).data('imageid'));
			                }
			            });

			            $('#<?php echo $id; ?>').val(imageList.join(','));
					}
					...
					<?php else : ?>
						$('#<?php echo $id; ?>').siblings('.noo-thumb-wrapper').sortable({
							stop: updateOrder_<?php echo $id; ?>
						});
					<?php endif; ?>
					...
					$('#<?php echo $id; ?>_upload').on('click', function(event) {
						...
						wp_media_frame.on('update', function(){
							...
							library.map( function( attachment ) {
								...
								noo_thumb_wraper.append('<img src="' + attachment.url + '" alt="" data-imageid="' + attachment.id + '" />');

								noo_thumb_wraper.sortable({
									stop: updateOrder_<?php echo $id; ?>
								});
							});
		...
		case 'media':
			...
			if ( $val != '' ) {
				$file_ids = explode( ',', $val );
			}
			echo '<input type="hidden" multiple="multiple" name="noo_meta_boxes[' . $id . ']" id="' . $id . '" value="' . ( $meta ? $meta : '' ) . '" style="margin-bottom:10px" id="' . $id . '" />';
			echo '<ul class="noo-thumb-wrapper">';
			if (!empty($file_ids)) :
			foreach ( $file_ids as $file_id ) :
    			$pdf_file = wp_get_attachment_url( $file_id ); 
    			if( $pdf_file ) : ?>
    				<?php $filetype=wp_check_filetype(basename(wp_get_attachment_url( $file_id)));?>
					<?php if (in_array($filetype['ext'], $check)): ?>
						<li id="<?php echo esc_attr($file_id) ?>" class="file">
		    			<a href="<?php echo wp_get_attachment_url($file_id); ?>" target="_blank">
		    				<?php echo basename(wp_get_attachment_url( $file_id)); ?>
	    				</a>
	    				<?php echo '<i class="fa fa-trash remove_file"></i>' ?>
	    				</li>
	    				<?php else: ?>
	    				<?php echo '<span>Allowed upload  <strong>' .$std. '</strong>. Please contact administrator supported</span>' ?>
					<?php endif ?>
			...
			<script>
				jQuery(document).ready(function($) {
					...
					$('#<?php echo esc_attr($id); ?>_upload').on('click', function(event) {
						...
						wp_media_frame.on('select', function(){
						    ...
						    selection.map( function( attachment ) {
						        ...
						        a.push(attachment.id);
						        noo_thumb_wraper.append('<li><a href="'+ attachment.url +'" target="_blank">'+ attachment.filename +'</a><li/>');
								
						    });
						    b = a.toString();

						    noo_upload_btn.siblings('#<?php echo esc_attr($id); ?>').val(b);
					...
					/* Remove File when click */
					$('.remove_file').click(function(event){
						event.preventDefault();
						var $noo_remove_btn = $(this);
						var id = $(this).parent().attr('id');
						var value = $(this).parent().parent().parent().find('input#_pdf_file').val();
						var arr_val = value.split(",");
						var index   = $.inArray(id,arr_val);
						if(-1 != index){
							arr_val.splice(index,1);
						}
						arr_val = arr_val.toString();
						$(this).parent().parent().find('input#_pdf_file').val(arr_val);
						$noo_remove_btn.closest('.file').remove();
		...
		case 'select':
			...
			$tag_multiple = ( !empty( $field['multiple'] ) ? '[]' : '' );
			echo'<select id='.$id.' name="noo_meta_boxes[' . $id . ']' . $tag_multiple . '" ' . $multiple . '>';

			if ( !empty( $field['option_none'] ) ) {
				echo '<option value="">' . sprintf( esc_html__( 'Select %s', 'noo' ), $field['label'] ) . '</option>';
			}
			foreach ( $field['options'] as $option ) {
				...
				if ( $meta == $opt_value || ( is_array( $meta ) && in_array( $opt_value, $meta ) ) ) echo ' selected="selected"'; 
	...
	foreach ( $_POST['noo_meta_boxes'] as $key=>$val ) {
		if( is_array( $val ) ) {
			...
			for( $index = 0; $index < $count; $index++ ) {
				// $val[$index] = stripslashes( $val[$index] );
				$val[$index] = stripslashes( htmlspecialchars( $val[$index] ) );
			}
		} else {
		...
		$val = apply_filters( 'noo_sanitize_meta_' . $key, $val );
...
if (!function_exists('noo_json_decode')) :
	function noo_json_decode( $json_str = '' ) {
		...
		if( !is_string($json_str) ) return array( $json_str );
		$maybe_json = json_decode($json_str, true);
		
		if( !is_array( $maybe_json ) ) {
			if( $json_str == '""' ) return array();

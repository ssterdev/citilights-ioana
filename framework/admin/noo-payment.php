	...
	class NooPayment {
		...
		public function __construct() {
			...
			if( is_admin() ) {
				...
				add_filter('re_setting_tabs', array(&$this,'add_setting_payment_tab'), 99);
				add_action('re_setting_payment', array(&$this,'payment_settings'));
			}
		...
		public function add_meta_boxes() {
			...
			} elseif( re_get_agent_setting('noo_membership_type', 'membership') == 'submission' ) {
				$meta_box['fields'][] = array(
						'id' => "_payment_type",
						'label' => __( 'Payment Type', 'noo' ),
						'desc' => '',
		...
		public function settings_sub_menu() {
			global $submenu;
			$permalink = re_setting_page_url('payment');
			$submenu['edit.php?post_type=noo_order'][] = array( 'Settings', 'edit_posts', $permalink );
		...
		public function add_setting_payment_tab( $tabs ) {
			$tabs['payment'] = __('Payment','noo');
			return $tabs;
		}
		...
		public function payment_settings() {
			?>
			<div class="wrap">
			<h3><?php _e('Payment Settings', 'noo'); ?></h3>
			...
			if( $order['status'] != $order_status ) {
				...
				if( $order_status == 'completed' ) {
					...
					if( NooMembership::is_membership() ) {
						...
						$user_email = get_post_meta( $order['agent_id'], "_noo_agent_email", true);
						...
						$order['payment_type'] = esc_attr( _get_post_meta( $order_id, '_payment_type', true ) );
		...
		public static function create_new_order( $payment_type = '', $billing_type = '', $item_id = '', $total_price = '', $agent_id = '', $title = '' ) {
			...
			update_post_meta( $order_ID, '_currency_code', re_get_property_setting('currency') );
